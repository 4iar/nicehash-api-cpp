...
/**
 * \brief Get detailed current stats for provider for all algorithms.
 *
 * Get the detailed current stats for provider for all algorithms (data refreshed every 30 seconds).
...
std::string NiceHashApi::getDetailedProviderStats (std::string address){
    ...
    std::string endpoint = "https://api.nicehash.com/api?method=stats.provider.ex&addr=";
